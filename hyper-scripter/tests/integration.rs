...
fn test_tags() {
    ...
    assert_eq!(MSG_JS, run("-f o/super_tag -").unwrap());

    assert_eq!(MSG, run(".1").expect("標籤篩選把舊的腳本搞爛了！"));

    run("tesjs").expect_err("標籤沒有篩選掉不該出現的腳本！可能是上上個操作把設定檔寫爛了");
    run("tags o/all").unwrap();
    run("tags no-hidden=").expect("沒有成功刪掉 no-hidden 標籤");
    run("tesjs").expect("沒吃到設定檔的標籤？");
    run("tags o/test").unwrap();
...
fn test_mv() {
    ...
    run(format!(
        "e . -c js -t test --no-template | echo \"{}\"",
        MSG
    ))
    .unwrap();
    ...
    run("-f o/test -").expect("用 mv 修改標籤把舊標籤搞丟了？");
    run("mv -a 1 -t o/hide").unwrap();
    run("-f o/test -").expect_err("用 mv 覆寫標籤失敗？");
}
...
fn test_edit_same_name() {
    ...
    assert_eq!(MSG, run("-f o/hide -").unwrap(), "腳本被撞名的編輯搞爛了？");
}
...
fn test_rm() {
    ...
    assert_eq!(MSG, run("-f o/removed test").unwrap());
    assert_eq!(
        MSG,
        run("-f o/test-tag test").expect("刪除沒有保留本來的標籤？")
    );

    assert_eq!(
        "你匿",
        run("-f o/removed,^test-tag -").expect("就算是匿名腳本也不該真的被刪掉！")
    );
    ...
    assert_eq!(
        "不要刪我 QmmmmQ",
        run("-f o/removed my/namespace/super-test").unwrap()
    );
    let file_path = run("-f o/removed which -").unwrap();
    ...
    run("rm * -f o/all --purge").expect("未能消滅掉一切");

    assert!(!check_exist(&["longlive.sh"]));
    run("-f o/all which").expect_err("沒有確實消滅掉一切");
}
...
fn test_append_tags() {
    ...
    run("e -t o/no-append no-append-test | echo 不要給我打標籤").unwrap();
    ...
    assert_eq!(
        "附加標籤",
        run("-f o/append apptest").expect("標籤沒附加上去？")
    );
    assert_eq!("不要給我打標籤", run("-f o/no-append apptest").unwrap());

    run("-f o/no-append mv no-append-test -t eventually-append").unwrap();
    assert_eq!(
        "不要給我打標籤",
        run("-f o/eventually-append apptest").expect("標籤沒被 mv 附加上去？")
    );
    assert_eq!(
        "不要給我打標籤",
        run("-f o/no-append apptest").expect("標籤被 mv 弄壞了？")
    );
}
...
fn test_mandatory_tags() {
    ...
    run("tags o/prj1").unwrap();
    assert_ls(vec!["prj1/t", "prj1/src/t"], None);
    assert_ls(vec!["prj1/src/t", "prj2/src/t"], Some("o/src"));
