use crate::args::{self, print_help, History, List, Root, Subs};
use crate::config::{Config, NamedTagFilter};
use crate::error::{Contextable, Error, Result};
use crate::extract_help;
use crate::list::{fmt_list, DisplayIdentStyle, DisplayStyle, ListOptions};
use crate::query::{self, ScriptQuery};
...
pub async fn main_with_args(args: &[String]) -> Result<Vec<Error>> {
...
async fn main_inner(root: Root) -> Result<MainReturn> {
    let conf = Config::get()?;
    let (pool, init) = crate::db::get_pool().await?;
