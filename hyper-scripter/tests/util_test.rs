...
fn test_import() {
    ...
    run("-f o/something-evil which -").expect_err("標籤匯入錯了？");
    run("tags all").unwrap();

    assert!(check_exist(&[".gitignore"]));

    assert_ls_len(16, Some("o/all"));
...
fn test_collect() {
    ...
    run("-f o/innate which myinnate").expect("還沒跑 collect 就壞掉了？");
    run("-f o/my which mytest").expect("還沒跑 collect 就壞掉了？");
    run("thisisacolltest").expect_err("還沒收集就出現了，嚇死");

    run("collect").unwrap();
    assert_eq!(
        run("-f o/this thisisacolltest").unwrap(),
        "這是一個收集測試"
    );
    assert_eq!(run("-f o/is thisisacolltest").unwrap(), "這是一個收集測試");
    assert_eq!(run(".10").unwrap(), "這是一個匿名收集測試");
    run("-f o/innate which myinnate").expect_err("跑了 collect 沒有刪成功");
    run("-f o/my which =my/test").expect_err("跑了 collect 沒有刪成功");
